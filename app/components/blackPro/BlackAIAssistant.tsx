...
function localAnswer(input: string, context: BlackAIContext) {
  ...
  return "I’m Black AI. I can explain the trading interface, risk controls and Orderly-powered workflows. Connect the Black AI backend to enable live account-aware analysis and tool calls.";
}

export default function BlackAIAssistant({ symbol, open, onClose }: { symbol: string; open: boolean; onClose: () => void }) {
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", content: "Black AI online. I can explain the terminal, risk controls and Orderly trading workflows. I will not invent market or account data." },
  ]);
  ...
  const send = async (text = input) => {
    ...
    try {
      if (endpoint) {
        ...
        if (!response.ok) throw new Error(`Black AI endpoint returned ${response.status}`);
      ...
      setMessages((current) => [...current, { role: "assistant", content: "Black AI could not reach its backend. Your trading interface remains independent and Orderly continues to be the source of truth." }]);
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <aside className="black-ai-panel" aria-label="Black AI assistant">
      <div className="black-ai-header">
        <div className="black-ai-title"><span className="black-ai-icon"><Bot size={16} /></span><div><strong>BLACK AI</strong><small>TRADING ASSISTANT</small></div></div>
      ...
      <form className="black-ai-input" onSubmit={(event) => { event.preventDefault(); void send(); }}>
        <input value={input} onChange={(event) => setInput(event.target.value)} placeholder="Ask Black AI…" aria-label="Ask Black AI" />

...
const LeftNavUI: FC<LeftNavUIProps> = (props) => {
  ...
  return (
    <button onClick={showModal} className={props?.className} aria-label="Open navigation menu" style={{ zoom: "1.2" }}>
      <VectorIcon />
    </button>
  );
};
...
const LeftNavSheet = modal.create<LeftNavUIProps>((props) => {
  ...
  return (
    <Sheet open={visible} onOpenChange={onOpenChange}>
      <SheetContent side="left" className="oui-w-[292px] oui-bg-base-8 oui-border-r oui-border-line-12" closeable closeableSize={24} closeOpacity={0.54}>
          ...
          <div className="oui-flex oui-h-[calc(100vh-130px)] oui-flex-col oui-items-start oui-overflow-y-auto oui-px-1">
            {Array.isArray(props?.menus) && props.menus.length > 0 && (
              <div className="oui-w-full oui-space-y-1">
                {props.menus.map((item) => (
                  <NavItem item={item} key={`item-${item.name}`} onLinkClick={hide} />
                ))}
              </div>
            )}
            {Array.isArray(props?.externalLinks) && props.externalLinks.length > 0 && (
              <>
                ...
                <div className="oui-w-full oui-space-y-1">
                  {props.externalLinks.map((item) => <ExternalNavItem item={item} key={`external-${item.name}`} />)}
                </div>
              </>
            )}
          </div>
...
const NavItem: FC<NavItemProps> = ({ item, onLinkClick }) => {
  ...
  const className = "oui-flex oui-items-center oui-gap-3 oui-rounded-md oui-px-3 oui-py-3 oui-w-full oui-transition-colors hover:oui-bg-base-7 hover:oui-text-primary oui-no-underline";
  const content = <><div className="oui-flex-shrink-0">{icon}</div><div className="oui-flex-1 oui-text-sm oui-font-semibold oui-text-base-contrast-80">{name}</div>{trailing}</>;

  if (customRender) {
    return <button type="button" onClick={onLinkClick} className={`${className} oui-bg-transparent oui-border-none`}>{customRender({ name, href })}</button>;
  }
  if (target) {
    return <a href={href} target={target} rel={target === "_blank" ? "noopener noreferrer" : undefined} onClick={onLinkClick} className={className}>{content}</a>;
  }
...
const ExternalNavItem: FC<ExternalNavItemProps> = ({ item }) => (
  <a href={item.href} target={item.target || "_blank"} rel="noopener noreferrer" className="oui-flex oui-items-center oui-justify-between oui-rounded-md oui-px-3 oui-py-3 oui-w-full oui-text-sm oui-font-semibold hover:oui-bg-base-7 oui-no-underline">

...
const getCustomMenuItems = (): MainNavItem[] => {
  ...
  try {
    ...
    for (const pair of menuPairs) {
      ...
      validCustomMenus.push({ name, href, target: "_blank" });
    }
...
const getEnabledMenus = (allMenuItems: MenuConfigItem[], defaultEnabledMenus: MenuConfigItem[]) => {
  ...
  try {
    ...
    for (const menuId of enabledMenuIds) {
      ...
      if (menuItem) enabledMenus.push(menuItem);
    }
...
const getPnLBackgroundImages = (): string[] => {
  ...
  if (useCustomPnL) {
    ...
    if (isNaN(customPnLCount) || customPnLCount < 1) {
      return [withBasePath("/pnl/poster_bg_1.png"), withBasePath("/pnl/poster_bg_2.png"), withBasePath("/pnl/poster_bg_3.png"), withBasePath("/pnl/poster_bg_4.png")];
    }
    const customPosters: string[] = [];
    for (let i = 1; i <= customPnLCount; i++) customPosters.push(withBasePath(`/pnl/poster_bg_${i}.webp`));
...
export const useOrderlyConfig = () => {
  ...
  const appIcons = useMemo<AppLogos>(() => ({
    ...
    secondary: {
      img: getRuntimeConfigBoolean("VITE_HAS_SECONDARY_LOGO") ? withBasePath("/logo-secondary.webp") : withBasePath("/orderly-logo-secondary.svg"),
    },
  }), []);
  ...
  return useMemo<OrderlyConfig>(() => {
    ...
    const bottomNavMenus = enabledMenus.filter((menu) => supportedBottomNavMenus.includes(menu.id)).map((menu) => ({
      name: menu.name,
      href: menu.href,
      target: menu.target,
    ...
    mainNavProps.customRender = (components) => (
      <Flex justify="between" className="oui-w-full">
        <Flex itemAlign="center" className={cn("oui-gap-3", "oui-overflow-hidden")}>
          <Link to="/" className="black-dex-integrated-brand">
            {getRuntimeConfigBoolean("VITE_HAS_SECONDARY_LOGO") ? (
              <img src={withBasePath("/logo-secondary.webp")} alt="Black DEX" style={{ height: isMobile ? "30px" : "32px" }} />
            ) : components.title}
          </Link>
          <div className="black-dex-integrated-nav">{components.mainNav}</div>
        </Flex>

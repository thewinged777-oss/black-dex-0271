...
const getCustomMenuItems = (): MainNavItem[] => {
  ...
  if (!customMenusEnv || typeof customMenusEnv !== "string" || customMenusEnv.trim() === "") return [];
  try {
    return customMenusEnv.split(";").map((pair) => pair.trim()).filter(Boolean).map((pair) => {
      const [name, href] = pair.split(",").map((item) => item.trim());
      if (!name || !href) return null;
      return { name, href, target: "_blank" };
    }).filter((item): item is MainNavItem => Boolean(item));
  } catch { return []; }
};

const getEnabledMenus = (allMenuItems: MenuConfigItem[], defaultEnabledMenus: MenuConfigItem[]) => {
  const enabledMenusEnv = getRuntimeConfig("VITE_ENABLED_MENUS");
  if (!enabledMenusEnv || typeof enabledMenusEnv !== "string" || enabledMenusEnv.trim() === "") return defaultEnabledMenus;
  try {
    const enabledMenus = enabledMenusEnv.split(",").map((id) => id.trim()).map((id) => allMenuItems.find((item) => item.id === id)).filter((item): item is MenuConfigItem => Boolean(item));
    return enabledMenus.length > 0 ? enabledMenus : defaultEnabledMenus;
  } catch { return defaultEnabledMenus; }
};
...
const getPnLBackgroundImages = (): string[] => {
  ...
  if (useCustomPnL) {
    ...
    if (!isNaN(customPnLCount) && customPnLCount >= 1) return Array.from({ length: customPnLCount }, (_, i) => withBasePath(`/pnl/poster_bg_${i + 1}.webp`));
  }
...
const getBottomNavIcon = (menuId: string) => {
  switch (menuId) {
    ...
    default: throw new Error(`Unsupported menu id: ${menuId}`);
  }
};
...
export const useOrderlyConfig = () => {
  ...
  const tradingViewConfig = useMemo<TradingPageProps["tradingViewConfig"]>(() => createTradingViewConfig(themeConfigSource), [themeConfigSource]);
  const sharePnLConfig = useMemo<TradingPageProps["sharePnLConfig"]>(() => ({
    backgroundImages: getPnLBackgroundImages(), color: "rgba(255,255,255,.98)", profitColor: "rgba(41,223,169,1)", lossColor: "rgba(245,97,139,1)", brandColor: "rgba(255,255,255,.98)", refLink: typeof window !== "undefined" ? window.location.origin : undefined, refSlogan: getRuntimeConfig("VITE_ORDERLY_BROKER_NAME") || "Orderly Network",
  }), []);
  ...
  return useMemo<OrderlyConfig>(() => {
    const allMenuItems: MenuConfigItem[] = [
      ...
      { id: "Campaigns", href: "", name: t("extend.tradingLeaderboard.campaigns"), target: "_blank", customRender: () => <CampaignsNavTitle title={t("extend.tradingLeaderboard.campaigns")} /> },
    ];
